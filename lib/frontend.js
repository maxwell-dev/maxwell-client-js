...
class Frontend extends maxwell_utils_1.Listenable {
    ...
    request(path, payload, headers) {
        return this._connection.waitOpen(this._options.waitOpenTimeout).then(() => {
            return this._connection
                .request(this._createReqReq(path, payload, headers), this._options.defaultRoundTimeout)
                .then((result) => {
    ...
    _newPullTask(topic, offset) {
        ...
        const pullTask = this._connection
            .request(this._createPullReq(topic, offset), this._options.defaultRoundTimeout)
            .then((value) => {
