...
const ActionHandler_1 = __importDefault(require("./ActionHandler"));
class Frontend extends Listenable_1.default {
    constructor(endpoints, connectionManager, options) {
        super();
        this._endpoints = endpoints;
        this._connectionManager = connectionManager;
        this._options = options;
        this._subscriptionManager = new SubscriptionManager_1.default();
        this._queueManager = new QueueManager_1.default(this._options.queueCapacity || 1024);
    ...
    _connectToFrontend() {
        this._resolveEndpoint().then((endpoint) => {
            ...
            this._connection.addListener(Event_1.default.ON_CONNECTED, this._onConnectToFrontendDone.bind(this));
            this._connection.addListener(Event_1.default.ON_ERROR, this._onConnectToFrontendFailed.bind(this));
            this._connection.addListener(Event_1.default.ON_DISCONNECTED, this._onDisconnectFromFrontendDone.bind(this));
            this._connection.addListener(Event_1.default.ON_MESSAGE, this._onAction.bind(this));
        }, (reason) => {
    ...
    _disconnectFromFrontend() {
        ...
        this._connection.deleteListener(Event_1.default.ON_CONNECTED, this._onConnectToFrontendDone.bind(this));
        this._connection.deleteListener(Event_1.default.ON_ERROR, this._onConnectToFrontendFailed.bind(this));
        this._connection.deleteListener(Event_1.default.ON_DISCONNECTED, this._onDisconnectFromFrontendDone.bind(this));
        this._connection.deleteListener(Event_1.default.ON_MESSAGE, this._onAction.bind(this));
    ...
    _newPullTask(topic, offset) {
        ...
        const pullTask = this._connection
            ...
            .catch((reason) => {
            if (reason instanceof TimeoutError_1.default) {
    ...
    _onAction(doReq) {
        ...
        if (callback !== undefined && this._connection !== null) {
            try {
                callback(new ActionHandler_1.default(doReq, this._connection));
            }
            catch (e) {
    ...
    _createDoReq(action, headers) {
        const doReq = new maxwell_protocol_1.msg_types.do_req_t({
            type: action.type,
            value: JSON.stringify(action.value ? action.value : {}),
            traces: [new maxwell_protocol_1.msg_types.trace_t()],
        });
    ...
    _createWatchReq(actionType) {
        return new maxwell_protocol_1.msg_types.watch_req_t({ type: actionType });
    }
    _createUnwatchReq(actionType) {
        return new maxwell_protocol_1.msg_types.unwatch_req_t({ type: actionType });
    }
}
