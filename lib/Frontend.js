...
class Frontend extends internal_1.Listenable {
    ...
    _connectToFrontend() {
        this._assignEndpoint().then((endpoint) => {
    ...
    async _assignEndpoint() {
        if (!this._options.masterEnabled) {
            return Promise.resolve(this._nextEndpoint());
        }
        const master = new internal_1.Master(this._endpoints, this._connectionManager, this._options);
        try {
    ...
    _newPullTask(topic, offset) {
        ...
        if (queue.isFull()) {
            ...
            this._onMsgs.get(topic)(offset - (0, internal_1.asOffset)(1));
            return;
        }
